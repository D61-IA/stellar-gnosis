...
def get_paper_info(url, source_website):
    ...
    try:
        ...
        if source_website == "acm":
            url = ""
            if (
                bs4obj.find("a", {"title": "Buy this Book"})
                or bs4obj.find("a", {"ACM Magazines"})
...
def analysis_url(url):
    ...
    # from IEEE
    elif url.startswith("https://ieeexplore.ieee.org/document/") or url.startswith(
    ...
    elif url.startswith("https://link.springer.com/chapter/") or url.startswith(
...
def find_author_list_from_IEEE(bs4obj):
    ...
    while array_count != 0:
        ...
        if text[i] == "}":
            ...
            if bracket_count == 0:
                author_list.append(
                    find_author_from_IEEE_author_info(text[bracket_start:i])
                )
        if text[i] == "]":
...
def get_ddl_from_spg(url):
    if "springer.com/chapter/" in url:
        download_link = url.replace(
            "springer.com/chapter/", "springer.com/content/pdf/", 1
        )
        download_link = download_link + ".pdf"
        return download_link
    elif "springer.com/article/" in url:
        download_link = url.replace(
            "springer.com/article/", "springer.com/content/pdf/", 1
        )
        download_link = download_link + ".pdf"
        return download_link
    else:
